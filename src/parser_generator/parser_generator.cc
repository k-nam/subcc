...
ContextFreeLanguage::ContextFreeLanguage(int terminalEnd, int nonTerminalEnd) : terminalEnd_(terminalEnd), nonTerminalEnd_(nonTerminalEnd),
																				isFinal_(false) {}

ContextFreeLanguage::Production::Production(Production const &rhs) : key_(rhs.key_), head_(rhs.head_), body_(rhs.body_) {}

ContextFreeLanguage::Production::Production(Production &&rhs) : key_(rhs.key_), head_(rhs.head_), body_(std::move(rhs.body_)) {}
...
int ContextFreeLanguage::add(int key, int inHead, int inSymbol, ...)
{
	...
	while (true)
	{
		...
		{ // nonnegative, in range, and not the reserved value
	...
	while (!finished)
	{
		...
		for (unsigned i = 0; i < productions_.size(); ++i)
		{
			...
			for (j = 0; j < curPro.body_.size(); ++j)
			{
				...
				{
					cout << "i is " << i << "j is " << j;
					cout << "calcul firstSet error; symbol is " << curPro.body_[j] << endl;
					exit(0);
				}
				bool hasEpsilon = false;
			...
			if (j == curPro.body_.size())
			{ // body can derive empty string!
	...
	while (!finished)
	{
		...
		for (unsigned i = 0; i < productions_.size(); ++i)
		{
			...
			for (j = 0; j < curPro.body_.size(); ++j)
			{
				...
				{
					cout << "i is " << i << "j is " << j;
					cout << "calcul followSet error; symbol is " << curPro.body_[j] << endl;
					exit(0);
				}
				set<int>::const_iterator iter;
...
SlrParserGenerator::LR0Item::LR0Item(LR0Item const &rhs) : production_(rhs.production_), dot_(rhs.dot_) {}

SlrParserGenerator::LR0Item::LR0Item(LR0Item &&rhs) : production_(std::move(rhs.production_)), dot_(rhs.dot_) {}
	...
	while (pointer < textLength)
	{
		...
		case Action::Shift:
		{
			cout << "Parser: shift " << inText[pointer] << " "
				 << "from state " << curState << " to state " << doThis.second << endl;
		...
		case Action::Reduce:
		{
			...
			curState = gotoTable_[curState][curPro.head_ - language_.terminalEnd_];
			stack.push_back(curState);
			cout << "Parser: reduce with " << doThis.second << " goto " << curState << endl;
			break;
		}
		case Action::Accept:
		{
...
SlrParserGenerator::parse(int inToken)
{
	...
	if (actionTable_[currentState_][inToken].empty())
	{ // empty slot
		cout << "Parser: empty slot error\n";
	...
	case Action::Reduce:
	{
		...
		// Be careful of gotoTable index
		currentState_ =
			gotoTable_[currentState_][curPro.head_ - language_.terminalEnd_];
		stack_.push_back(currentState_);
	...
	while (!stack.empty())
	{
		...
		for (int i = 0; i < language_.nonTerminalEnd_; ++i)
		{
			...
			if (nextItemSet.empty())
				continue;
			if (lR0ItemSetToState_.count(nextItemSet) == 0)
			{ // newly found
...
void SlrParserGenerator::checkConflict()
{
	...
	for (unsigned i = 0; i < actionTable_.size(); ++i)
	{ // for all States
		...
		{ // for all terminals
			...
			if (shiftPrece > reducePrece)
			{ // do shift
				solution = Action::Shift;
			}
			else if (shiftPrece < reducePrece)
			{ // do reduce
				solution = Action::Reduce;
			}
			else if (association_.count(shiftPrece) == 0)
			{ // not set: shift
	...
	for (unsigned i = 0; i < gotoTable_.size(); ++i)
	{ // for all States
								 ...
								 set<LR0Item> &result) const
{
	...
	for (iter = source.begin(); iter != source.end(); ++iter)
	{
		if (iter->dot_ == iter->production_.body_.size()) // end dot
			continue;
		if (iter->production_.body_[iter->dot_] == inSymbol)
		{							// right move
									...
									set<LR0Item> &closure) const
{
	...
	while (!finished)
	{
		...
		for (auto iter = items.begin(); iter != items.end(); ++iter)
		{
			...
			{
				if (closure.insert(targetProductions[i]).second)
				{ // inserted!
					finished = false;
				}
			}
...
int SlrParserGenerator::getPrecedence(Production inPro) const
{
	...
	{
		if (inPro.body_[i] < language_.terminalEnd_)
		{ // is a terminal
...
Lr1ParserGenerator::LR1Item::LR1Item(LR1Item const &rhs) : LR0Item(rhs), lookahead_(rhs.lookahead_) {}

Lr1ParserGenerator::LR1Item::LR1Item(LR1Item &&rhs) : LR0Item(std::move(rhs)), lookahead_(rhs.lookahead_)
{
	compareCount_++;
}
...
void Lr1ParserGenerator::build()
{
	...
	while (!stack.empty())
	{
		...
		for (int i = 0; i < language_.nonTerminalEnd_; ++i)
		{
			...
			if (LR1ItemSetToState_.count(nextItemSet) == 0)
			{ // newly found
				nextState = makeNewState();
		...
		for (auto iter = items.begin(); iter != items.end(); ++iter)
		{
			...
			for (unsigned i = 0; i < targetProductions.size(); ++i)
			{
				...
				for (iter2 = firstSet.begin(); iter2 != firstSet.end(); ++iter2)
				{
					...
					if (closure.insert(std::move(newItem)).second)
					{ // inserted!
						finished = false;
					}
				}
...
void LalrParserGenerator::build()
{
		 ...
		 ++iter)
	{
		...
		if (i == unionArray.size())
		{ // This item set is first discovered now
	...
	for (unsigned i = 0; i < oldActionTable.size(); ++i)
	{ // i: old state
...
void LalrParserGenerator::buildFast()
{
	...
	for (state_iter = lR0ItemSetToState_.begin();
		 state_iter != lR0ItemSetToState_.end(); ++state_iter)
	{ // for all states
		set<LR0Item>::const_iterator item_iter;
		for (item_iter = state_iter->first.begin();
			 item_iter != state_iter->first.end(); ++item_iter)
		{ // for each items
			if (item_iter->dot_ != 0)
			{ // is a kernel
		 ...
		 kernel_iter != kernelArray.end(); ++kernel_iter)
	{
		...
		for (int i = 0; i < language_.nonTerminalEnd_; ++i)
		{
				 ...
				 item_iter != targetLR1Kernel.end(); ++item_iter)
			{
				...
				if (item_iter->lookahead_ == kDollarMarker)
				{ // lookahead = $
					propagates[*kernel_iter].insert(std::move(target));
				}
				else
				{ // real Token lookahead
