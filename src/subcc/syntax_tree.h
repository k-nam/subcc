...
private:
	...
	SyntaxTree &operator=(SyntaxTree const &);
...
{
public:
	IdNode(string const &lexeme, int lineNum) : lexeme_(lexeme), lineNum_(lineNum) {}
...
{
public:
	FunctionNode(int productionKey, int maxStackSize) : NonTerminalNode(productionKey), maxStackSize_(maxStackSize) {}
...
{
public:
	StmtNode(int productionKey, bool isReturning) : NonTerminalNode(productionKey), isReturning_(isReturning) {}
...
{
public:
	TypedNode(int productionKey, shared_ptr<Type const> type) : NonTerminalNode(productionKey), type_(type) {}
	//virtual ~TypedNode() { }
