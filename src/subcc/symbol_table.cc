...
Symbol::Symbol(enum SymbolKinds symbolKind, string const &lexeme,
			   shared_ptr<Type const> type) : symbolKind_(symbolKind), lexeme_(lexeme), type_(type)
{
	++objectCount_;
}
...
Constant::Constant(char charValue) : Symbol(SYMBOL_CONSTANT_CHAR, string(stringOfInt(charValue)),
											getBaseType(BASETYPE_CHAR))
{
	constant_.charValue_ = charValue;
}

Constant::Constant(short shortValue) : Symbol(SYMBOL_CONSTANT_SHORT, string(stringOfInt(shortValue)),
											  getBaseType(BASETYPE_SHORT))
{
	constant_.shortValue_ = shortValue;
}

Constant::Constant(int intValue) : Symbol(SYMBOL_CONSTANT_INT, string(stringOfInt(intValue)),
										  getBaseType(BASETYPE_INT))
{
	constant_.intValue_ = intValue;
}

Constant::Constant(string const &lexeme) : Symbol(SYMBOL_CONSTANT_STRING, lexeme,
...
SymbolTable::SymbolTable(GlobalSymbolTable const &globalTable,
						 SymbolTable *parent, bool isEnvironment) : globalTable_(globalTable), parent_(parent),
...
SymbolTable::SymbolTable(GlobalSymbolTable const &globalTable) : globalTable_(globalTable), parent_(NULL), offset_(0),
...
void SymbolTable::insert(shared_ptr<Symbol> symbol, int index)
{
	...
	{
		if (mapping_.count(symbol->getLexeme()) != 0)
	...
	else if (symbol->isStringLiteral())
	{
		...
		int offset = const_cast<GlobalSymbolTable &>(globalTable_).saveStringLiteral(lexeme.substr(1, lexeme.length() - 2));
