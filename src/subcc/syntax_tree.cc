...
SyntaxTree::SyntaxTree(SyntaxTreeNode const *root, shared_ptr<GlobalSymbolTable const> symtab) : root_(root), symbolTable_(symtab) {}
...
ExprNode::ExprNode(int key, shared_ptr<Type const> type, bool isConstant) : TypedNode(key, type), isAssignable_(false), isConstant_(isConstant),
																			rvalueIndex_(-1) {}

ExprNode::ExprNode(int key, shared_ptr<Type const> type, bool isAssignable,
				   bool isConstant) : TypedNode(key, type), isAssignable_(isAssignable),
									  isConstant_(isConstant), rvalueIndex_(-1) {}

/****************** LvalueNode methods ***************/

LvalueNode::LvalueNode(int key, shared_ptr<Type const> type) : ExprNode(key, type, true, false),
