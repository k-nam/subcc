...
Address::Address() : addressType_(AddressType_Null), symbolTableIndex_(-1) {}

Address::Address(
	enum AddressType addressType, int index) : addressType_(addressType), symbolTableIndex_(index) {}
...
FunctionStartInstruction::FunctionStartInstruction(string const &functionName, int stackSize) : Instruction(Instruction_Function_Start), functionName_(functionName),
...
FunctionEndInstruction::FunctionEndInstruction(
	string const &functionName, int stackSize) : Instruction(Instruction_Function_End), functionName_(functionName),
												 stackSize_(stackSize) {}
...
LabelInstruction::LabelInstruction(int label) : Instruction(Instruction_Label), label_(label) {}
...
AssignInstruction::AssignInstruction(enum InstructionType instType,
									 Address const &target, Address const &source) : Instruction(instType), target_(target), source_(source) {}
...
CopyInstruction::CopyInstruction(Address const &target, Address const &source) : AssignInstruction(Instruction_Assign_Copy, target, source) {}

/*** UnaryAssignInstruction ***/

UnaryAssignInstruction::UnaryAssignInstruction(Address const &target,
											   enum UniOp op, Address const &source) : AssignInstruction(Instruction_Assign_Unary, target, source), operator_(op) {}
...
BinaryAssignInstruction::BinaryAssignInstruction(Address const &target,
												 Address const &srcA, enum BinOps op, Address const &srcB) : AssignInstruction(Instruction_Assign_Binary, target, srcA),
...
FunctionCallInstruction::FunctionCallInstruction(Address const &returnValue,
												 Address const &function) : Instruction(Instruction_Function_Call),
...
ReturnInstruction::ReturnInstruction() : Instruction(Instruction_Return), returnValue_(Address()) {}

ReturnInstruction::ReturnInstruction(Address const &returnValue) : Instruction(Instruction_Return), returnValue_(returnValue) {}
...
ParameterInstruction::ParameterInstruction(Address const &parameter) : Instruction(Instruction_Parameter), parameter_(parameter) {}
...
PopInstruction::PopInstruction(int popSize) : Instruction(Instruction_Pop), popSize_(popSize) {}
...
JumpInstruction::JumpInstruction(int targetLabel) : BranchInstruction(Instruction_Jump, targetLabel) {}

/*** UnaryBranchInstruction ***/

UnaryBranchInstruction::UnaryBranchInstruction(Address const &cond,
											   int targetLabel) : BranchInstruction(Instruction_Branch_Unary, targetLabel), condition_(cond) {}
...
UnaryNotBranchInstruction::UnaryNotBranchInstruction(Address const &cond,
													 int targetLabel) : BranchInstruction(Instruction_Branch_Unary_Not, targetLabel),
																		condition_(cond) {}
...
BinaryBranchInstruction::BinaryBranchInstruction(Address const &condA,
												 enum BinOps op, Address const &condB, int targetLabel) : BranchInstruction(Instruction_Branch_Binary, targetLabel),
...
IndexedSrcCopyInstruction::IndexedSrcCopyInstruction(
	Address const &target, Address const &source, Address const &index) : AssignInstruction(Instruction_Copy_Indexed_Src, target, source),
																		  index_(index) {}
...
IndexedTargetCopyInstruction::IndexedTargetCopyInstruction(
	Address const &target, Address const &index, Address const &source) : AssignInstruction(Instruction_Copy_Indexed_Target, target, source),
																		  index_(index) {}
...
ThreeAdressCode::ThreeAdressCode(shared_ptr<GlobalSymbolTable const> symbolTable) : symbolTable_(symbolTable) {}
...
ThreeAdressCodeGenerator::ThreeAdressCodeGenerator(shared_ptr<SyntaxTree const> syntaxTree) : root_(syntaxTree->getRoot()), symbolTable_(syntaxTree->getSymbolTable()),
												   ...
												   int trueNext, int falseNext)
{
	...
	{		 // expr (save r-value to x)
		...
		{
			char cond = dynamic_pointer_cast<Constant const>(symbolTable_->lookUp(node->getRvalueIndex()))->getCharValue();
			if (cond)
...
void ThreeAdressCodeGenerator::translateExpression(ExprNode const *node, bool saveRvalue)
{
	...
	{		  // ++expr
		...
		Address result(getAddress(node->getRvalueIndex()));
		int oneIndex = std::const_pointer_cast<GlobalSymbolTable>(symbolTable_)->insert(shared_ptr<Symbol>(new Constant(1)));
		enum BinOps op;
	...
	case 242: // lvalue [ expr ]
	{
		...
		int typeWidth = dynamic_pointer_cast<ArrayType const>(
							...
							->getElementType()
							->getWidth();
		int widthIndex = std::const_pointer_cast<GlobalSymbolTable>(symbolTable_)->insert(shared_ptr<Symbol>(new Constant(typeWidth)));
		...
		if (oldIndex.isNull())
		{ // lvalue is not indexed (a single Name)
		...
		if (saveRvalue)
		{ // If rvalue is needed, do an indexed copy
	...
	case 243: // expr . ID
	{
		...
		int offsetIndex = std::const_pointer_cast<GlobalSymbolTable>(symbolTable_)->insert(shared_ptr<Symbol>(new Constant(recordOffset)));
